...
class Player
  ...
  def execute_command comm, arg=""
    if @editing.empty? == false
      log :debug, "#{@name} editor com input: #{comm}"
      begin
        ...
        if !edit_arr[0]
          ...
          new_arg = edit_arr[0].filter(arg) # returns what the players arg translates into
  ...
  def cmd_dig command_table_entry, arg
    ...
    if r == nil
        Room.dig_rooms(in_room.vnum, Area.gen_vnum(in_room.vnum/1000), i)
    else
        Room.dig_rooms(in_room.vnum, r.vnum, i)
    end
    return true
  end
  ...
  def cmd_asave c_t_e, arg
    text_to_player "All rooms saved." + ENDL
    save_all_rooms
  ...
  def buildwalk(dir)
    ...
    if r == nil
      new_room = Room.dig_rooms(in_room.vnum, Area.gen_vnum(in_room.vnum/1000), dir)
  ...
  def cmd_look command_table_entry, arg
    ...
    text_to_player "#{in_room.name}" + ENDL if in_room.name != "Default Name"
    m.view(self)
